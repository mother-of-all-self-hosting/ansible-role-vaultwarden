...
#
# Tags look like `v<Vaultwarden version>-<release>`:
#
# - if defaults/main.yml points at a Vaultwarden version that has never been
#   released, the release counter restarts at 0 (`v1.37.2-0`)
# - otherwise the counter is incremented (`v1.37.2-1`), but only if something
...
version="$(sed -nE 's|^vaultwarden_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Vaultwarden version from $defaults_path"
	exit 1
fi
